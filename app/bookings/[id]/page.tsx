// app/bookings/[id]/page.tsx
import { prisma } from "@/app/lib/prisma";
import { notFound } from "next/navigation";
import Link from "next/link";
import { getServerSession } from "next-auth";
import { authConfig } from "@/auth.config";
import { ApproveButton } from "../_components/ApproveButton";
import RejectButton from "../_components/RejectButton";
import ShippingForm from "./_components/ShippingForm";
import ReturnForm from "./_components/ReturnForm";
import { openChatFromBookingAction } from "./actions";

// ===== Helpers =====
const fmt = (d?: Date | null) =>
  d
    ? d.toLocaleString("pl-PL", {
        day: "2-digit",
        month: "2-digit",
        year: "numeric",
        hour: "2-digit",
        minute: "2-digit",
      })
    : "—";

const badge = (label: string, cls: string) => (
  <span className={`text-xs px-2 py-1 rounded border ${cls}`}>{label}</span>
);

// ✅ plural polaco: ocena/oceny/ocen
function plOceny(n: number) {
  if (n === 1) return "ocena";
  if (n % 10 >= 2 && n % 10 <= 4 && !(n % 100 >= 12 && n % 100 <= 14)) return "oceny";
  return "ocen";
}

// ===== Status rezerwacji =====
const statusLabel: Record<string, string> = {
  PENDING: "Oczekuje na akceptację",
  CONFIRMED: "Potwierdzona",
  AWAITING_PAYMENT: "Oczekuje na płatność",
  PAID: "Opłacona",
  CANCELLED: "Odrzucona",
};

const statusClass: Record<string, string> = {
  PENDING: "bg-amber-100 text-amber-800 border-amber-200",
  CONFIRMED: "bg-emerald-100 text-emerald-800 border-emerald-200",
  AWAITING_PAYMENT: "bg-blue-100 text-blue-800 border-blue-200",
  PAID: "bg-emerald-100 text-emerald-800 border-emerald-200",
  CANCELLED: "bg-rose-100 text-rose-700 border-rose-200",
};

// ===== Status wysyłki =====
const shippingLabel: Record<string, string> = {
  NOT_REQUIRED: "Nie wymaga wysyłki",
  PENDING: "Oczekuje na przygotowanie",
  READY: "Gotowe do wysyłki",
  SHIPPED: "W drodze",
  DELIVERED: "Dostarczono",
  LOST: "Zagubiono / uszkodzono",

  RETURN_PENDING: "Oczekuje na zwrot",
  RETURNED: "Zwrot otrzymany",

  CANCELLED: "Anulowano",
};

const shippingClass: Record<string, string> = {
  NOT_REQUIRED: "bg-gray-100 text-gray-700 border-gray-200",
  PENDING: "bg-amber-100 text-amber-800 border-amber-200",
  READY: "bg-sky-100 text-sky-800 border-sky-200",
  SHIPPED: "bg-blue-100 text-blue-800 border-blue-200",
  DELIVERED: "bg-emerald-100 text-emerald-800 border-emerald-200",
  LOST: "bg-rose-100 text-rose-700 border-rose-200",

  RETURN_PENDING: "bg-fuchsia-100 text-fuchsia-800 border-fuchsia-200",
  RETURNED: "bg-emerald-100 text-emerald-800 border-emerald-200",

  CANCELLED: "bg-gray-100 text-gray-700 border-gray-200",
};

// ===== Status zwrotu =====
const returnLabel: Record<string, string> = {
  RETURN_PENDING: "Oczekuje na zwrot",
  SHIPPED: "Zwrot w drodze",
  DELIVERED: "Zwrot dostarczony",
  RETURNED: "Zwrot otrzymany",
  CANCELLED: "Anulowano",
};

const returnClass: Record<string, string> = {
  RETURN_PENDING: "bg-amber-100 text-amber-800 border-amber-200",
  SHIPPED: "bg-blue-100 text-blue-800 border-blue-200",
  DELIVERED: "bg-emerald-100 text-emerald-800 border-emerald-200",
  RETURNED: "bg-emerald-100 text-emerald-800 border-emerald-200",
  CANCELLED: "bg-gray-100 text-gray-700 border-gray-200",
};

// ===== Traducciones método de entrega =====
const metodoEnvioLabel: Record<string, string> = {
  RECOGIDA_LOCAL: "Odbiór osobisty",
  ENVIO_CORREOS: "Wysyłka pocztą",
  MENSAJERIA: "Kurier",
  OTRO: "Inna metoda",
};

export default async function BookingPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;

  const session = await getServerSession(authConfig);
  const userId = session?.user?.id ?? null;

  const booking = await prisma.booking.findUnique({
    where: { id },
    include: {
      listing: {
        select: {
          id: true,
          title: true,
          metodoEnvio: true,
          fianza: true,
          userId: true,
        },
      },
      renter: { select: { id: true, name: true } },
    },
  });

  if (!booking) return notFound();

  const isOwner = !!userId && booking.listing?.userId === userId;
  const isRenter = !!userId && booking.renterId === userId;

  // ✅ Opiniones SOBRE el inquilino (como RENTER)
  const renterAgg = await prisma.review.aggregate({
    where: { revieweeId: booking.renterId, role: "RENTER" },
    _avg: { rating: true },
    _count: { rating: true },
  });

  const renterAvg = renterAgg._avg.rating ?? 0;
  const renterCount = renterAgg._count.rating ?? 0;

  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Szczegóły rezerwacji</h1>
        <Link href="/bookings" className="text-blue-600 underline">
          ← Wróć
        </Link>
      </div>

      {/* ===== Información general ===== */}
      <section className="p-4 border rounded bg-white space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-sm text-gray-500">Ogłoszenie</div>
            <Link
              href={`/listing/${booking.listingId}`}
              className="text-blue-700 hover:underline font-medium"
            >
              {booking.listing?.title ?? "Ogłoszenie"}
            </Link>

            <div className="text-sm text-gray-600 mt-1">
              {fmt(booking.startDate)} — {fmt(booking.endDate)}
            </div>

            <div className="text-sm text-gray-500 mt-2 flex flex-wrap items-center gap-x-2 gap-y-1">
              <span>
                Najemca:{" "}
                <span className="font-medium">{booking.renter?.name ?? "—"}</span>
              </span>

              {/* ✅ rating del inquilino */}
              {renterCount > 0 ? (
                <Link
                  href={`/users/${booking.renterId}`}
                  className="inline-flex items-center gap-1 text-xs text-gray-600 hover:underline"
                  title="Zobacz profil i opinie"
                >
                  <span aria-hidden>★</span>
                  <span className="font-medium">{renterAvg.toFixed(1)}</span>
                  <span className="text-gray-400">
                    ({renterCount} {plOceny(renterCount)})
                  </span>
                </Link>
              ) : (
                <span className="text-xs text-gray-400">(Brak ocen)</span>
              )}
            </div>

            {/* ✅ Botón abrir chat (para owner o renter). Reutiliza UNA sola conversación */}
            {(isOwner || isRenter) && booking.status !== "CANCELLED" && (
              <form action={openChatFromBookingAction} className="mt-3">
                <input type="hidden" name="bookingId" value={booking.id} />
                <button
                  type="submit"
                  className="px-4 py-2 rounded bg-emerald-600 text-white hover:bg-emerald-700"
                >
                  Otwórz czat
                </button>
              </form>
            )}
          </div>

          {badge(
            statusLabel[booking.status] ?? booking.status,
            statusClass[booking.status] ??
              "bg-gray-100 text-gray-800 border-gray-200"
          )}
        </div>
      </section>

      {/* ===== Wysyłka (solo si CONFIRMED) ===== */}
      {booking.status === "CONFIRMED" && (
        <section className="p-4 border rounded bg-white space-y-4">
          <h2 className="text-lg font-semibold">Wysyłka</h2>

          <div className="flex flex-wrap items-center gap-2">
            {badge(
              shippingLabel[booking.shippingStatus] ?? booking.shippingStatus,
              shippingClass[booking.shippingStatus] ??
                "bg-gray-100 text-gray-800 border-gray-200"
            )}

            <span className="text-sm text-gray-600">
              Preferowana metoda dostawy:{" "}
              <strong>
                {booking.listing?.metodoEnvio
                  ? metodoEnvioLabel[booking.listing.metodoEnvio] ??
                    booking.listing.metodoEnvio
                  : "—"}
              </strong>
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-gray-500">Przewoźnik:</span>{" "}
              {booking.carrier ?? "—"}
            </div>
            <div>
              <span className="text-gray-500">Numer śledzenia:</span>{" "}
              {booking.trackingNumber ?? "—"}
            </div>
            <div>
              <span className="text-gray-500">Wysłano dnia:</span>{" "}
              {fmt(booking.shippedAt)}
            </div>
            <div>
              <span className="text-gray-500">Dostarczono dnia:</span>{" "}
              {fmt(booking.deliveredAt)}
            </div>
          </div>

          {/* Formulario SOLO para owner */}
          {isOwner && (
            <>
              <hr className="my-3" />
              <ShippingForm
                bookingId={id}
                initial={{
                  shippingStatus: booking.shippingStatus ?? "PENDING",
                  carrier: booking.carrier ?? null,
                  trackingNumber: booking.trackingNumber ?? null,
                  shippedAt: booking.shippedAt ?? null,
                  deliveredAt: booking.deliveredAt ?? null,
                }}
              />
            </>
          )}
        </section>
      )}

      {/* ===== Zwrot (solo si CONFIRMED) ===== */}
      {booking.status === "CONFIRMED" && (
        <section className="p-4 border rounded bg-white space-y-4">
          <h2 className="text-lg font-semibold">Zwrot</h2>

          <div className="flex flex-wrap items-center gap-2">
            {badge(
              returnLabel[booking.returnStatus] ?? booking.returnStatus,
              returnClass[booking.returnStatus] ??
                "bg-gray-100 text-gray-800 border-gray-200"
            )}
            <span className="text-sm text-gray-600">
              Dane zwrotu (uzupełnia najemca)
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
            <div>
              <span className="text-gray-500">Przewoźnik:</span>{" "}
              {booking.returnCarrier ?? "—"}
            </div>
            <div>
              <span className="text-gray-500">Numer śledzenia:</span>{" "}
              {booking.returnTrackingNumber ?? "—"}
            </div>
            <div>
              <span className="text-gray-500">Wysłano dnia:</span>{" "}
              {fmt(booking.returnShippedAt)}
            </div>
            <div>
              <span className="text-gray-500">Dostarczono dnia:</span>{" "}
              {fmt(booking.returnDeliveredAt)}
            </div>
          </div>

          {/* Formulario SOLO para inquilino */}
          {isRenter && (
            <>
              <hr className="my-3" />
              <ReturnForm
                bookingId={id}
                initial={{
                  returnStatus: booking.returnStatus ?? "RETURN_PENDING",
                  returnCarrier: booking.returnCarrier ?? null,
                  returnTrackingNumber: booking.returnTrackingNumber ?? null,
                  returnShippedAt: booking.returnShippedAt ?? null,
                  returnDeliveredAt: booking.returnDeliveredAt ?? null,
                }}
              />
            </>
          )}
        </section>
      )}

      {/* ===== Acciones (solo propietario) ===== */}
      {isOwner && booking.status === "PENDING" && (
        <section className="p-4 border rounded bg-white space-y-3">
          <h2 className="text-lg font-semibold">Akcje</h2>
          <div className="flex gap-3">
            <ApproveButton bookingId={id} />
            <RejectButton bookingId={id} />
          </div>
        </section>
      )}
    </div>
  );
}

...
export default function PendingTasksBell({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        ...
        if (active) { setTasks(data.tasks); setError(false); }
  ...
  return <details ref={panel} className="relative" onToggle={event => { if (event.currentTarget.open) reload.current(); }}>
    ...
    <div className="fixed inset-x-3 top-16 z-50 rounded-lg border bg-white p-3 shadow-lg md:absolute md:inset-x-auto md:right-0 md:top-full md:mt-2 md:w-96">
      ...
      <ul className="max-h-[60vh] overflow-y-auto divide-y">{tasks?.map(task => <li key={task.id}><Link prefetch={false} href={task.href} onClick={() => { if (panel.current) panel.current.open = false; }} className="block rounded p-3 hover:bg-gray-50">
      ...
      <p className="border-t pt-2 text-xs text-gray-500">Zadania znikają po wykonaniu i odświeżeniu listy. Daty w czasie polskim.</p>
    </div>
  </details>;
}
